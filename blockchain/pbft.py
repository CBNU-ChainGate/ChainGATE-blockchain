...
class PBFT:
    ...
    def changing_primary(self):
        global primary_N, primary
        primary_N = (primary_N+1) % len(self.blockchain.nodes)
        primary = list(self.blockchain.nodes)[self.changing_primary()]
        print(f'Primary Node is "{primary}"')

    def primary_change_protocol(self):
        global view, primary, consensus_failed, start_time, request_data, consensus_nums
        # consensus_failed = False  # 합의 실패 플래그 초기화

        # 새로운 primary 노드 선택
        self.changing_primary()
        ...
        if consensus_nums > 4:
            ...
            consensus_nums += 1
            self.send(primary, {'type': 'REQUEST', 'data': request_data})
    ...
    def wait_msg(self, caller):
        """모든 노드의 응답을 받을 때까지 대기"""
        global get_pre_msg, get_commit_msg, node_id, primary
        if caller == 'prepare':
            get_pre_msg += 1     # 응답을 받은 노드 개수 저장
            if node_id == primary and get_pre_msg == self.node_len:
                get_pre_msg = 0
                print("*****GET ALL MESSAGE*****")
                return False
            elif get_pre_msg == self.node_len-1:
                get_pre_msg = 0
                print("*****GET ALL MESSAGE*****")
                return False
        elif caller == 'commit':
            get_commit_msg += 1     # 응답을 받은 노드 개수 저장
            if get_commit_msg == self.node_len:
                get_commit_msg = 0
                print("*****GET ALL MESSAGE*****")
                return False
        return True

    def validate_preprepare(self, preprepare_message):
        ...
        if preprepare_message['view'] != view or preprepare_message['seq'] != self.blockchain.len+1:
            return False
        return True
    ...
    def handle_request(self):
        ...
        try:
            ...
            if node_id == primary:
                ...
                self.blockchain.len = self.blockchain.get_block_total()
                N = self.blockchain.len + 1
                print("len: ", end='')  # debugging
                print(self.blockchain.len)  # debugging
                ...
                for node in self.blockchain.nodes:
                    if node == node_id:
                        continue
                    preprepare_thread = Thread(target=self.send, args=(node, {
                        'type': 'PREPREPARE',
        ...
        try:
            ...
            if self.validate_preprepare(message):
                ...
                for node in self.blockchain.nodes:
                    if node == node_id:
                        continue
                    prepare_thread = Thread(target=self.send, args=(node, {
                        'type': 'PREPARE',
                        'view': view+1,
                        'seq': message['seq'],
    ...
    def handle_prepare(self):
        ...
        try:
            ...
            if self.wait_msg('prepare'):  # 모든 노드한테서 메세지를 받을 때까지 기다리기
                consensus_done[2] += 1
            ...
            if len(prepare_msg_list) > 2/3 * (self.node_len-1):
                ...
                for node in self.blockchain.nodes:
                    if node == node_id:
                        continue
                    commit_thread = Thread(target=self.send, args=(node, {
                        'type': 'COMMIT',
                        'view': view+2,
                        'seq': message['seq'],
                        # 'digest': message['digest'],
    ...
    def handle_commit(self):
        ...
        while consensus_done[2] < self.node_len-1:
            pass
        try:
            message = request.get_json()
            log.append(message)         # commit 메세지 수집
            if self.wait_msg('commit'):  # 모든 노드한테서 메세지를 받을 때까지 기다리기
                return jsonify({'message': 'Wait the message!'}), 404
            print("~~COMMIT~~")  # Debugging
            commit_msg_list = [m for m in log if m['type'] == 'COMMIT' and m['view']
                               == message['view'] and m['seq'] == message['seq']]
            if len(commit_msg_list) > 2/3 * self.node_len:
    ...
    def reply_request(self):
        self.blockchain.add_transaction(request_data)
        last_block = self.blockchain.last_block
        if self.blockchain.create_block(self.blockchain.hash(last_block)):
